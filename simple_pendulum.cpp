 ...
 * @brief Main file for numerical simulation of "wall hop" modeled as simple pendulum.
 ...
 */
 ...
 public:
   double dT;
   double time_limit; //タイムリミットになったら終了？or速度がマイナスになったら終了？
   ...
   simple_pendulum(condition_param input_pc){
     ...
     point_tmp.x=(-1)*pc.L*cos(pc.theta*M_PI);
     point_tmp.y=(-1)*pc.L*sin(pc.theta*M_PI);
     abs_mass_point.push_back(point_tmp);
   }

   void renew_state(){  //考えるのめんどいからオイラー法
     x_2dot=pc.g*cos(pc.theta-phi);
     x_dot-=dT*x_2dot;
     phi_dot=x_dot/pc.L;
     phi+=dT*phi_dot;
     time.push_back(time.back()+dT);
     point_tmp.x=(-1)*pc.L*cos(pc.theta*M_PI-phi);
     point_tmp.y=(-1)*pc.L*sin(pc.theta*M_PI-phi);
     abs_mass_point.push_back(point_tmp);
   }
 };
 ...
 int main(int argc, char const *argv[]){
   ...
   for(unsigned int i=0;i<sp.abs_mass_point.size();i++){
     x_tmp[0]=sp.abs_mass_point[i].x;
     x_tmp[0]=sp.abs_mass_point[i].y;
